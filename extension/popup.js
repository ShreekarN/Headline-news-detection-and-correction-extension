...
document.getElementById('checkBtn').addEventListener('click', async ()=>{
  ...
  try {
    ...
    payload.page_url = tab.url || '';
    // normalize image list to absolute URLs (fixes /wp-content/... relative paths)
    const pageBase = tab.url || '';
    payload.image_urls = (images || []).map(i => {
      const src = i && i.src ? i.src : i;
      if (!src) return null;
      try { return new URL(src, pageBase).href; } catch (e) { return src; }
    }).filter(Boolean);
    ...
    if (t && Object.keys(t).length) {
      ...
      html += `<div style="margin-top:6px"><b>Suggested (paraphrased lead):</b> ${esc(t.suggested_title || '')}</div>`;
    } else {
